...
const fetchRecentData = async () => {
  ...
  try {
    ...
    if (!isDemo) {
      ...
      // Demo mode
      const localSongs = JSON.parse(localStorage.getItem('musicroll_songs') || '[]')
      recentSongs.value = localSongs.slice(0, 4)
      
      const localSetlists = JSON.parse(localStorage.getItem('musicroll_setlists') || '[]')
...
<template>
  <div class="dashboard-panel glass-panel">
    <div class="dashboard-header mb-4">
      <h2 class="gradient-text-primary">Bem-vindo, {{ user?.email?.split('@')[0] || 'Músico' }}!</h2>
      <p class="text-muted">Aqui está um resumo do seu acervo musical.</p>
    </div>

    <div class="quick-actions-row mb-4">
      <button @click="$emit('navigate', 'song_create')" class="btn btn-primary btn-action">
        <PlusCircle :size="18" /> Nova Cifra
      </button>
      <button @click="$emit('navigate', 'songs_list')" class="btn btn-secondary btn-action">
        <Music :size="18" /> Todas as Cifras
      </button>
      <button @click="$emit('navigate', 'setlists')" class="btn btn-secondary btn-action">
    ...
    <div v-else class="dashboard-content-grid">
      ...
      <div class="dashboard-card">
        <h3 class="dashboard-card-title"><Clock :size="18" /> Últimas Cifras Adicionadas</h3>
          ...
          >
            <div class="recent-meta">
              <span class="recent-title">
                {{ song.title }} 
                <span v-if="Number(song.bpm || 120) > 180" title="Acima de 180 BPM">🌶️</span>
              </span>
      ...
      <!-- Ultimos Setlists -->
      <div class="dashboard-card">
        <h3 class="dashboard-card-title"><Clock :size="18" /> Setlists Recentes</h3>
        ...
        <div v-else class="recent-list">
          ...
          >
            <div class="recent-meta">
              ...
              <span class="recent-subtitle">{{ new Date(setlist.created_at || Date.now()).toLocaleDateString('pt-BR') }}</span>
            </div>
